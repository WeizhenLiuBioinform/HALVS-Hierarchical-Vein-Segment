...
parser.add_argument('--partial-labeled-id-path', type=str, required=True)
...
#             nn.MaxPool2d( kernel_size=3, stride=1, padding=1),
#             )
    poolinglabel_model = Poolinglabel(4)
...
#                                                       output_device=local_rank, find_unused_parameters=False)
    ...
    t_img, t_mask = trainset_l[0]#[3, 256, 256],[256, 256]
    org_u_left=0
    ...
    weigth_avgpool = nn.AvgPool2d(kernel_size=32, stride=4, padding=14, count_include_pad=False)
    temp_map = torch.zeros(4,64,64)
    for _i_ in range(64):
        for _j_ in range(64):
...
#             cfg['clean_rate']=0.1
        ...
        for i, ((img_x, mask_x),
            ...
            with torch.no_grad():
                mask_patches = torch.stack([mask_x==i for i in range(4)], dim=1)#1,19,801,801
...
#                     all_class_max_labeled_unconf[_class_]=avg_TP_conf[_class_].item()
            ...
            if(epoch==0):     
                loss_u_p = (loss_x+loss_multi_x ) / 2.0  
                loss = loss_u_p + loss_p          
...
#                 total_mask_ratio.update(0)            
            if(epoch>0):
                ...
                with torch.no_grad():
                    ...
                    mlabels_u_w_cm1, mlabels_u_w_cm2, mlabels_u_w_fp = mlabels_u_w_s.chunk(3)
                    # print(torch.squeeze(mlabel_cfeats_s1).shape)#[1, 40401, 19] [1, 40401, 19] [1, 40401, 19]
                ...
                loss_u_p = (loss_x+loss_multi_x + loss_u_s1 * 0.25 + loss_u_s2 * 0.25 + loss_u_w_fp * 0.5 + multiloss_s1 * 0.1 + multiloss_s2 * 0.1 + multiloss_u_w_fp * 0.1 ) / 3.3
                loss = loss_u_p + loss_p
                torch.distributed.barrier()
                optimizer.zero_grad()
                loss.backward()
