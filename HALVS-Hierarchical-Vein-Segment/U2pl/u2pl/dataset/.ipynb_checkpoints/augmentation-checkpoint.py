...
def generate_aug_data(data):
    batch_size, _, im_h, im_w = data.shape

    new_data = []
    for i in range(batch_size):
        aug_data = transforms.ColorJitter(0.5, 0.5, 0.5, 0.25)(data[i])
        aug_data = transforms.RandomGrayscale(p=0.2)(aug_data)
        aug_data = transforms.GaussianBlur(kernel_size=(5, 5), sigma=(0.1, 2.0))(aug_data)
        aug_data = aug_data.unsqueeze(0)
